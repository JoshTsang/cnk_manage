		...
		$.getJSON(url, function(data){
			if (undefined === data.succ) {
			    ...
			    $.each(data, function(i, dish){
			      dishes.dishes[i] = new Dish(dish.id, dish.name, dish.ename, dish.shortcut, dish.price, dish.unitName, dish.description, dish.sortPrintName, dish.index);
			      $("#dishes").append('<tr><td>' + dish.index + '</td><td>' + dish.name +  '</td><td>' + dish.ename + '</td><td>' + 
			      					 ...
			      					 dish.description + '</td><td>' + dish.sortPrintName + '</td><td class="action"><a href="#">[修改] </a> <a href="#"> [删除]</a></td></tr>"');
			    });
			} else {
				//TODO err handle
			}
